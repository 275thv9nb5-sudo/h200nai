"""
Experiment A: YOLO26l 4ch + EQL v2 + small-image oversampling (FIXED 1280)
============================================================================
The proven eqlv2 recipe (LB 54.197, #21) + 3x oversampled small images.
NO multi_scale (that recipe lost -0.010 val / -4.4 LB, see experiment log).

Prerequisite: scripts/build_oversample_dataset.py must have run.

Usage:
  python3 scripts/train_yolo26l_oversample.py                  # full (H200)
  python3 scripts/train_yolo26l_oversample.py --batch 2 --workers 0 --smoke
"""
...
OUTPUT_DIR = ROOT / "outputs" / "exp_yolo26l_4ch_oversample"
...
if __name__ == '__main__':
    ...
    model = YOLO(str(ROOT / 'yolo26l.pt'))
    n_params = sum(p.numel() for p in model.model.parameters()) / 1e6
    print(f"[Model] YOLO26l: {n_params:.1f}M params")
    ...
    if args.smoke:
        ...
        print("Exp A: YOLO26l 4ch + EQL v2 + small-image oversampling @1280")
    print("=" * 60)
